...
function EBook({reading}: { reading: any }) {
    return (
        <li className={buildReadingClassNames('physical-book', reading.status)}>
            <LinkToReading mms={reading.metadata.mms_id} title={thumbnail(reading.metadata.isbn, reading.metadata.title)} status={reading.status}/>
            <cite><LinkToReading mms={reading.metadata.mms_id} title={reading.metadata.title} status={reading.status}/></cite><br/>
